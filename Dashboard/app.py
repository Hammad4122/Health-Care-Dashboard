...
import altair as alt                   # For data visualization (advanced charts)
...
def run_app():
    ...
    # ========================================================
    #                DEPARTMENT DATASET SECTION
    # ========================================================
    elif selected_dataset == "Department":
        st.header("Department Dataset")
        st.divider()

        # Group total and occupied beds by department
        ...
        with col1:
            ...
            st.bar_chart(
                data=grouped_department_total_beds,
                x='department',
                y='total_beds',
                x_label="Department",
                y_label="Total Beds",
        ...
        with col2:
            ...
            st.bar_chart(
                data=grouped_department_occupied_beds,
                x='department',
                y='occupied_beds',
                x_label='Department',
    ...
    # ========================================================
        ...
        # ---------------------------------------------
        #   Search Feature (Doctor ID / Name / Dept)
        # ---------------------------------------------
        ...
        if search_button:
            ...
            if not (user_search_input and user_department_search_input):
                ...
                # Both fields provided → filter by both
                if user_search_input.isdigit():
                    search_result = doctor.loc[
                        (doctor["doctor_id"] == int(user_search_input)) &
                        (doctor["department"].str.lower() == user_department_search_input.lower())
                    ]
                else:
                    search_result = doctor.loc[
                        (doctor['name'].str.lower() == user_search_input.lower()) &
                        (doctor["department"].str.lower() == user_department_search_input.lower())
                    ]
                st.dataframe(search_result, use_container_width=True)

        # Horizontal option menu for multiple graphs
        ...
        if selected_option == 'Graph 1':
            ...
            # Total patients seen by department
    ...
    # ========================================================
        ...
        # ---------------------------------------------
        #   Search Feature (Patient ID / Name / Dept)
        # ---------------------------------------------
        ...
        if search_button:
            ...
            if not (user_patient_search_input and user_patient_department_search_input):
                ...
                # Both name/id and department entered
                if user_patient_search_input.isdigit():
                    search_result = patient.loc[
                        (patient["patient_id"] == int(user_patient_search_input)) &
                        (patient["department"].str.lower() == user_patient_department_search_input.lower())
                    ]
                else:
                    search_result = patient.loc[
                        (patient['name'].str.lower() == user_patient_search_input.lower()) &
                        (patient["department"].str.lower() == user_patient_department_search_input.lower())
                    ]
                st.dataframe(search_result, use_container_width=True)

        # ---------------------------------------------
        #        Visualization Menu for Patients
        # ---------------------------------------------
        selected_option = option_menu(
            menu_title=None,
            options=['Graph 1', "Graph 2", "Graph 3"],
...
# ========================================================
#           Function to Import All Datasets
# ========================================================
def import_data():
    BASE_DIR = os.path.dirname(__file__)   # Get current directory

    # Load all cleaned CSV files from "cleaned_datasets" folder
